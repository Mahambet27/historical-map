...
export default function TimelinePage() {
  ...
  return <div className="content-page"><header className="page-hero"><p className="eyebrow"><span></span> Машина времени</p><h1>История Казахстана<br /><em>по эпохам</em></h1><p>Перемещайтесь по временной шкале и открывайте связанные территории, события и личности.</p></header><div className="content-toolbar"><input value={query} onChange={e=>setQuery(e.target.value)} type="search" placeholder="Найти эпоху или событие…"/><Link to="/demo">Показать на карте →</Link></div><div className="timeline-list">{filtered.map((item,index)=><article key={item[0]}><span>{String(index+1).padStart(2,"0")}</span><div><small>{item[1]}</small><h2>{item[0]}</h2><p>{item[2]}</p></div><Link to="/demo">Исследовать эпоху ↗</Link></article>)}</div></div>;
}
